...
public class Room: NSObject, ObservableObject, Loggable {
    ...
    var _blockProcessQueue = DispatchQueue(label: "EasyRTCSwift.engine.pendingBlocks",
                                           qos: .default)
                ...
                roomOptions: RoomOptions? = nil)
    {
        ...
        super.init()
        // log sdk & os versions
        log("sdk: \(EasyRTCSDK.version), os: \(String(describing: Utils.os()))(\(Utils.osVersionString())), modelId: \(String(describing: Utils.modelIdentifier() ?? "unknown"))")
